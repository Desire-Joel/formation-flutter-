...
main(){
  ...
  // });
  // print(album.id);
   List<Album> albums = albumss.map((json)=>Album.fromJson(json:json)).toList();
  //  albums.forEach((f)=> print("title: ${f.title}\n userId: ${f.userId}\n id: ${f.id}"));
  albums.where((al)=> al.title.contains("ia")).toList().forEach((f){
    print("title: ${f.title}\n userId: ${f.userId}\n id: ${f.id}");
  });
}
